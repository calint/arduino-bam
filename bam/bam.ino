...
void setup() {
  ...
  touch_screen.setRotation(TFT_ORIENTATION);
...
static inline void render_scanline(uint16_t *render_buf_ptr,
  ...
  for (int layer = 0; layer < sprites_layers; layer++) {
    ...
    for (int i = 0; i < len; i++, spr++) {
      if (spr->layer != layer || !spr->img || spr->scr_y > scanline_y || spr->scr_y + sprite_height <= scanline_y || spr->scr_x <= -sprite_width || spr->scr_x >= display_width) {
      ...
      if (flip_vert) {
        spr_img_ptr += (sprite_height - 1) * sprite_width - (scanline_y - spr->scr_y) * sprite_width;
      } else {
